...
var mri = getCjsExportFromNamespace(lib);
...
if (process.argv[2] === 'start') {
    ...
    console.error(index$1.$.bold().red("'sapper start' has been removed"));
...
prog.command('dev')
    ...
    .action(function (opts) { return index$1.__awaiter(void 0, void 0, void 0, function () {
        ...
        switch (_a.label) {
            ...
            case 1:
                ...
                try {
                    ...
                    watcher.on('ready', function (event) { return index$1.__awaiter(void 0, void 0, void 0, function () {
                        var exec;
                        return index$1.__generator(this, function (_a) {
                            switch (_a.label) {
                                case 0:
                                    if (!first_1) return [3 /*break*/, 3];
                                    console.log(index$1.$.bold().cyan("> Listening on http://localhost:" + event.port));
                    ...
                    watcher.on('invalid', function (event) {
                        ...
                        console.log("\n" + index$1.$.bold().cyan(changed) + " changed. rebuilding...");
                    });
                    watcher.on('error', function (event) {
                        var type = event.type, error = event.error;
                        console.log(index$1.$.bold().red("\u2717 " + type));
                        if (error.loc && error.loc.file) {
                            console.log(index$1.$.bold(path.relative(process.cwd(), error.loc.file) + " (" + error.loc.line + ":" + error.loc.column + ")"));
                        }
                        console.log(index$1.$.red(event.error.message));
                        if (error.frame)
                            console.log(error.frame);
                    });
                    watcher.on('fatal', function (event) {
                        console.log(index$1.$.bold().red("> " + event.message));
                        if (event.log)
                            console.log(event.log);
                    });
                    watcher.on('build', function (event) {
                        if (event.errors.length) {
                            console.log(index$1.$.bold().red("\u2717 " + event.type));
                            event.errors.filter(function (e) { return !e.duplicate; }).forEach(function (error) {
                                if (error.file)
                                    console.log(index$1.$.bold(error.file));
                            ...
                            console.log(index$1.$.bold().yellow("\u2022 " + event.type));
                            event.warnings.filter(function (e) { return !e.duplicate; }).forEach(function (warning) {
                                if (warning.file)
                                    console.log(index$1.$.bold(warning.file));
                            ...
                            console.log(index$1.$.bold().green("\u2714 " + event.type) + " " + index$1.$.gray("(" + index$1.format_milliseconds(event.duration) + ")"));
                        }
                    });
                }
                catch (err) {
                    console.log(index$1.$.bold().red("> " + err.message));
                    console.log(index$1.$.gray(err.stack));
                    process.exit(1);
                }
                return [2 /*return*/];
        }
        ...
        return index$1.__generator(this, function (_a) {
            switch (_a.label) {
                ...
                case 2:
                    ...
                    console.error("\n> Finished in " + index$1.elapsed(start) + ". Type " + index$1.$.bold().cyan("node " + dest) + " to run the app.");
                    return [3 /*break*/, 4];
                case 3:
                    err_1 = _a.sent();
                    console.log("" + index$1.$.bold().red("> " + err_1.message));
                    console.log(index$1.$.gray(err_1.stack));
                    process.exit(1);
                    return [3 /*break*/, 4];
    ...
    return index$1.__awaiter(void 0, void 0, void 0, function () {
        ...
        return index$1.__generator(this, function (_a) {
            switch (_a.label) {
                ...
                case 1:
                    _a.sent();
                    console.error("\n> Built in " + index$1.elapsed(start));
                    _a.label = 2;
                case 2: return [4 /*yield*/, Promise.resolve().then(function () { return require('./export.js'); })];
                case 3:
                    _export = (_a.sent()).export;
                    return [4 /*yield*/, Promise.resolve().then(function () { return require('./index2.js'); }).then(function (n) { return n.index; })];
                case 4:
                    ...
                    return [4 /*yield*/, _export({
                            ...
                            oninfo: function (event) {
                                console.log(index$1.$.bold().cyan("> " + event.message));
                            },
                            onfile: function (event) {
                                var size_color = event.size > 150000 ? index$1.$.bold().red : event.size > 50000 ? index$1.$.bold().yellow : index$1.$.bold().gray;
                                var size_label = size_color(index$1.left_pad(pb_1(event.size), 10));
                                var file_label = event.status === 200
                                    ? event.file
                                    : index$1.$.bold()[event.status >= 400 ? 'red' : 'yellow']("(" + event.status + ") " + event.file);
                                console.log(size_label + "   " + file_label);
                            }
                        })];
                case 5:
                    _a.sent();
                    console.error("\n> Finished in " + index$1.elapsed(start) + ". Type " + index$1.$.bold().cyan("npx serve " + dest) + " to run the app.");
                    return [3 /*break*/, 7];
                case 6:
                    err_2 = _a.sent();
                    console.error(index$1.$.bold().red("> " + err_2.message));
                    process.exit(1);
                    return [3 /*break*/, 7];
                ...
                case 1:
                    ...
                    return [4 /*yield*/, build({
                            ...
                            oncompile: function (event) {
                                ...
                                var c = function (txt) { return index$1.$.cyan(txt); };
                                var warnings = event.result.warnings;
                                if (warnings.length > 0) {
                                    banner += " with " + warnings.length + " " + (warnings.length === 1 ? 'warning' : 'warnings');
                                    c = function (txt) { return index$1.$.cyan(txt); };
                                }
                                console.log();
                                console.log(c("\u250C\u2500" + index$1.repeat('─', banner.length) + "\u2500\u2510"));
                                console.log(c("\u2502 " + index$1.$.bold(banner) + " \u2502"));
                                console.log(c("\u2514\u2500" + index$1.repeat('─', banner.length) + "\u2500\u2518"));
